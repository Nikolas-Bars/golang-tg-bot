...
func (s Storage) Save(page *storage.Page) (err error) {
	...
	// filepath.Join - грамотно готовит путь адаптируя и под мак и под windows
	filePath := filepath.Join(s.basePath, page.UserName)

	if err := os.MkdirAll(filePath, defaultPermission); err != nil {
		return err
	}
	// нам нужно уникальное имя для каждого файла
...
func (s *Storage) PickRandom(userName string)(page *storage.Page, err error) {
	...
	return s.decodeFile(filepath.Join(path, file.Name()))
}

func (s *Storage) IsExists(p *storage.Page) (bool, error) {
	fileName, err := fileName(p)
	if(err != nil) {
		return false, e.Wrap("can`t check exist", err)
	}
	...
	switch _, err := os.Stat(path); {
		case errors.Is(err, os.ErrNotExist):
			return false, nil
		case err != nil: 
			return false, e.Wrap(fmt.Sprintf("can`t check exist %s", path), err)
	}
...
func (s *Storage) decodeFile(filePath string)(*storage.Page, error) {
